...
namespace PlanningsTool.BLL.Services
{
    public class HolidaysService : IHolidaysService
    {
        ...
        {
            if (await CheckIfGeneratedYearExist(year))
            {
                throw new Exception($"Het jaar {year} bestaat al in de lijst");
            }
        ...
        public async Task<bool> CheckIfGeneratedYearExist(int year)
        {
